...
function computeWindowShading(vsaDeg, hsaDeg, awningDepth, awningGap, windowHeight, windowWidth, awningWidth) {
  ...
  // Negative sign: sun from the right (HSA>0) pushes shadow to the LEFT (-x)
...
function computeFloorPenetration(vsaDeg, sillHeight, shadedHeight, windowHeight) {
  ...
  if (tanVSA <= 0.001) {
    // Near-horizontal sun: clamp to a large but finite value (99 ft) for display
    var cap = 99;
    return { penetrationFromSill: cap, penetrationFromUnshadedTop: cap, maxPenetration: cap };
  }
...
// Shoelace formula for 2D polygon area (using x, y properties)
...
function computeSunlightPatch(params) {
  ...
  // Compute awning shadow bounds on window
  var halfWw = windowWidth / 2;
  var windowTop = sillHeight + windowHeight;
  var shadowBottom = windowTop - shading.shadedHeight;
  // Negative sign: sun from the right (HSA>0) pushes shadow to the LEFT (-x)
  var lateralOffset = -awningDepth * Math.tan(hsaRad);

  // The awning shadow on the window plane is a PARALLELOGRAM, not a rectangle.
  // At the top (y = windowTop, where awning meets wall at z=0): shadow edges at ±awningWidth/2
  // At the bottom (y = shadowBottom, front edge projection): edges shifted by lateralOffset
  var shadowLeftTop = -awningWidth / 2;
  var shadowRightTop = +awningWidth / 2;
  var shadowLeftBot = lateralOffset - awningWidth / 2;
  var shadowRightBot = lateralOffset + awningWidth / 2;

  // Check if shadow parallelogram overlaps window rectangle at all
  var shadowMinLeft = Math.min(shadowLeftTop, shadowLeftBot);
  var shadowMaxRight = Math.max(shadowRightTop, shadowRightBot);
  var shadowHitsWindow = shadowMaxRight > -halfWw + 0.01 && shadowMinLeft < halfWw - 0.01;

  // Decompose unshaded window opening into polygonal bands.
  // The awning shadow is a parallelogram with diagonal left/right edges
  // when the sun is at an oblique angle (lateralOffset ≠ 0).
  var bands = [];

  if (!shadowHitsWindow || shading.shadedHeight < 0.01) {
    // No effective shadow — full window is unshaded
    bands.push([
      { x: -halfWw, y: sillHeight },
      { x: halfWw,  y: sillHeight },
      { x: halfWw,  y: windowTop },
      { x: -halfWw, y: windowTop }
    ]);
  } else {
    // Bottom band: full width, below shadow bottom (still rectangular)
    if (shadowBottom > sillHeight + 0.01) {
      bands.push([
        { x: -halfWw, y: sillHeight },
        { x: halfWw,  y: sillHeight },
        { x: halfWw,  y: shadowBottom },
        { x: -halfWw, y: shadowBottom }
      ]);
    }

    // Left leak: polygon between window left edge and shadow's left diagonal edge.
    // Shadow left edge runs from (shadowLeftTop, windowTop) to (shadowLeftBot, shadowBottom).
    var leftLeak = [
      { x: -halfWw,       y: shadowBottom },
      { x: shadowLeftBot,  y: shadowBottom },
      { x: shadowLeftTop,  y: windowTop },
      { x: -halfWw,       y: windowTop }
    ];
    leftLeak = clipPolygonToHalfPlane(leftLeak, 'x', -halfWw, false);
    leftLeak = clipPolygonToHalfPlane(leftLeak, 'x', halfWw, true);
    if (leftLeak.length >= 3 && polygonArea2D(leftLeak) > 0.001) {
      bands.push(leftLeak);
    }

    // Right leak: polygon between shadow's right diagonal edge and window right edge.
    // Shadow right edge runs from (shadowRightTop, windowTop) to (shadowRightBot, shadowBottom).
    var rightLeak = [
      { x: shadowRightBot, y: shadowBottom },
      { x: halfWw,         y: shadowBottom },
      { x: halfWw,         y: windowTop },
      { x: shadowRightTop, y: windowTop }
    ];
    rightLeak = clipPolygonToHalfPlane(rightLeak, 'x', -halfWw, false);
    rightLeak = clipPolygonToHalfPlane(rightLeak, 'x', halfWw, true);
    if (rightLeak.length >= 3 && polygonArea2D(rightLeak) > 0.001) {
      bands.push(rightLeak);
    }
  }

  if (bands.length === 0) return empty;

  // Project each band (polygon) onto room surfaces
  var allFloor = [], allWall = [], allLeft = [], allRight = [];
  ...
  for (var b = 0; b < bands.length; b++) {
    ...
    floorPoly = clipPolygonToHalfPlane(floorPoly, 'x', halfRW, true);
    floorPoly = clipPolygonToHalfPlane(floorPoly, 'x', -halfRW, false);
    ...
    wallPoly = clipPolygonToHalfPlane(wallPoly, 'x', halfRW, true);
    wallPoly = clipPolygonToHalfPlane(wallPoly, 'x', -halfRW, false);
    ...
    // Convert to 3D and collect
    if (floorPoly.length >= 3) {
      allFloor.push(floorPoly.map(function(p) { return { x: p.x, y: 0, z: p.z }; }));
    }
    if (wallPoly.length >= 3) {
      allWall.push(wallPoly.map(function(p) { return { x: p.x, y: p.y, z: roomDepth }; }));
    }
    if (leftWallPoly.length >= 3) {
      allLeft.push(leftWallPoly.map(function(p) { return { x: -halfRW, y: p.y, z: p.z }; }));
    }
    if (rightWallPoly.length >= 3) {
      allRight.push(rightWallPoly.map(function(p) { return { x: halfRW, y: p.y, z: p.z }; }));
    }
  }

  return { floor: allFloor, wall: allWall, leftWall: allLeft, rightWall: allRight };
}
...
function recommendOptimalAwning(lat, lng, wallAzDeg, windowHeight, windowWidth, glazingType, locks, method) {
  locks = locks || {};
  method = method || 'nrel'; // 'nrel', 'ba', or 'susdesign'
  const isNorth = lat >= 0;
  const summerMonth = isNorth ? 5 : 11;
  const winterMonth = isNorth ? 11 : 5;
  ...
  // ── SusDesign: south-facing only — no E/W recommendation ─────────────
  if (method === 'susdesign' && orientationCategory === 'east-west') {
    return {
      depth: null, gap: null, widthExtra: null,
      type: {
        newbuild: 'No horizontal overhang recommendation for this orientation',
        retrofit: 'No horizontal overhang recommendation for this orientation',
        reasons: [
          'The SusDesign methodology (derived from NREL lookup tables) only provides recommendations for equator-facing windows (S ±60° in Northern Hemisphere).',
          'For east/west-facing walls, consider vertical fins, retractable awnings, exterior solar screens, or operable interior shading.',
          'Switch to NREL/ASHRAE or Building America method for a computed depth estimate.'
        ]
      },
      feasible: false, summerVSA: null, winterVSA: null,
      maxBeforeWinterBlocking: null,
      ...
      climateZone: climateZone,
      method: method,
      message: 'SusDesign does not provide recommendations for east/west-facing walls.'
    };
  }
  ...
  var gap, idealDepth, depth, idealGapValue;
  var depthExceedsSlider = false;
  ...
  var depthLocked = locks.depth !== undefined;
  var gapLocked = locks.gap !== undefined;
  ...
  if (isEquatorish && winter && winter.vsaDeg > 0) {
    ...
    if (designVSA && designVSA.vsaDeg > 0) {
      ...
      if (tanDesign > tanWinter) {
        ...
        if (depthLocked && gapLocked) {
          // Both locked — just use the locked values
          depth = locks.depth;
          gap = locks.gap;
          idealDepth = depth;
        } else if (depthLocked) {
          // Depth locked — compute optimal gap for this depth
          depth = locks.depth;
          idealDepth = depth;
          // Gap so winter shadow clears window top: gap = depth * tan(winterVSA)
          var idealGap = depth * tanWinter;
          idealGapValue = idealGap;
          gapExceedsSlider = idealGap > 2;
          gap = Math.round(Math.min(idealGap, 2) * 4) / 4;
        } else if (gapLocked) {
          // Gap locked — compute optimal depth for this gap
          gap = locks.gap;
          // depth = (windowHeight + gap) / tan(designVSA)
          idealDepth = (windowHeight + gap) / tanDesign;
          depthExceedsSlider = idealDepth > 6;
          depth = Math.round(Math.min(idealDepth, 6) * 4) / 4;
        } else {
          // Neither locked — original SusDesign calculation
  ...
  if (!usedDesignDate) {
    ...
    if (depthLocked) {
      ...
      idealDepth = (windowHeight + gap) / Math.tan(summerVSARad);

      if (method === 'ba' && orientationCategory === 'east-west') {
        // Building America approach: use south-facing depth for E/W walls,
        // supplemented with operable shading for gaps in coverage.
        var equatorAz = isNorth ? 180 : 0;
        var southDesignVSA = computeVSAForDateNoon(lat, lng, equatorAz, designMonth, designDay);
        if (southDesignVSA && southDesignVSA.vsaDeg > 0) {
          var southDepth = (windowHeight + gap) / Math.tan(southDesignVSA.vsaDeg * DEG);
          depthExceedsSlider = southDepth > 6;
          depth = Math.round(Math.min(southDepth, 6) * 4) / 4;
        } else {
          depthExceedsSlider = idealDepth > 6;
          depth = Math.round(Math.min(idealDepth, 6) * 4) / 4;
        }
      } else {
        // NREL/ASHRAE: full geometric depth for all non-north orientations.
        depthExceedsSlider = idealDepth > 6;
        depth = Math.round(Math.min(idealDepth, 6) * 4) / 4;
      }
    }
  ...
  // Width targets full window coverage at 10 AM and 2 PM on summer solstice,
  // with HSA capped at 45° per ASHRAE methodology. Same formula for all orientations.
  var widthMaxLimit = windowWidth + 5;
  var widthExceedsSlider = idealWidthExtra > widthMaxLimit;
  var widthExtra;
  if (locks.widthExtra !== undefined) {
    widthExtra = locks.widthExtra;
  } else {
    widthExtra = Math.round(Math.min(idealWidthExtra, widthMaxLimit) * 2) / 2;
  }
  ...
  if (isEquatorish) {
    ...
    if (method === 'ba') {
      newbuildType = 'South-sized overhang + operable shading';
      retrofitType = 'Fixed awning (south-sized) + exterior solar screen or blinds';
      reasons.push('Building America guidance: use a south-facing-sized overhang for partial coverage, supplemented with operable shading (blinds, retractable screens) for low-angle morning/evening sun.');
      reasons.push('The computed depth matches the equator-facing recommendation for this latitude, providing partial E/W coverage without over-sizing.');
    } else {
      // NREL/ASHRAE — full geometric depth
      newbuildType = 'Retractable awning, vertical fins, or combined approach';
      retrofitType = 'Retractable fabric awning or exterior solar screen';
      reasons.push('East/west-facing walls receive low-angle sun. The NREL Solar Radiation Data Manual notes that horizontal overhangs are not particularly effective for these orientations — consider supplementing with vertical fins, operable shading, or solar screens.');
    }
    if (depthExceedsSlider) {
      reasons.push('Full shading requires ' + idealDepth.toFixed(1) + ' ft depth (exceeds the 6 ft slider max). The computed depth provides partial coverage — a retractable or combined strategy is recommended.');
    }
    if (!feasible) {
      reasons.push('The summer shade depth exceeds the winter sun limit (' + (maxBeforeWinterBlocking ? maxBeforeWinterBlocking.toFixed(1) : '?') + ' ft) — a retractable awning avoids blocking useful winter sun.');
    }
  }
  ...
  if (depthExceedsSlider && isEquatorish) {
    reasons.push('Note: The ideal depth (' + idealDepth.toFixed(1) + ' ft) exceeds the 6 ft slider maximum. The slider has been set to the maximum.');
  }
  if (gapExceedsSlider) {
    reasons.push('Note: The ideal gap (' + (idealGapValue || 0).toFixed(1) + ' ft) exceeds the 2 ft slider maximum. The slider has been set to 2 ft.');
  }
  if (widthExceedsSlider) {
    reasons.push('Note: The ideal width extension (' + idealWidthExtra.toFixed(1) + ' ft) exceeds the slider maximum (' + widthMaxLimit.toFixed(0) + ' ft). The slider has been set to ' + widthMaxLimit.toFixed(0) + ' ft.');
  }
